...
`ATLAS` · `reg()` · `outlineRegion()` · `vgradeRegion()` · `drawGrid()` · `TRIW` · `triCorners()` · `triBary()` · `triIns()` · `decorTri()` · `paintTriFull()` · `paintTriTransition()` · `paintRoadHex()` · `paintRiverTri()` · `paintRiverMouth()` · `paintWaterfall()` · `paintBridge()` · `paintIcon()` · `buildAtlas()`
