...
function genViewer(){
  ...
  // масштаб вдвое меньше экрана — чтобы канва целиком влезала на телефоне
  const SC=Math.max(2,Math.floor(Math.min(innerWidth,innerHeight-60)/(S.W*2)));
  ...
  const frame=(now)=>{
    ...
    if(!done){
      ...
      carry+=(total/90)*(S.speed||1)*dt; // ~90 c на всю сборку при 1x — шаги можно рассмотреть
