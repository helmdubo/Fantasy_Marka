...
состояние проекта, итоги последней сессии и план — в `docs/HANDOFF.md`
(начинай новую сессию с него).
...
- Генерация мира → `src/02c_worldgen.js` (`genWorld`); графы мира → `S.world`
- Старт: фаза `S.phase==='scout'` (палатка) → `placeTownhall` → `'play'`
- Лес непроходим для жителей (`S.pass`), герои/рейдеры ходят (`S.passHero`)
